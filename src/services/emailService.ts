...
export const sendOrderNotificationWebhook = async (data: OrderNotificationData, type: 'customer' | 'admin' = 'customer'): Promise<boolean> => {
  try {
    ...
    console.log('✅ Webhook notification sent to Zapier')
    return true
  ...
  try {
    ...
    await fetch(webhookUrl, {
      ...
      body: JSON.stringify({
        ...
        admin_email: import.meta.env.VITE_ADMIN_EMAIL || 'orders@sweettripcandy.com',
        customer_name: data.customerName,
        order_number: data.orderNumber,
    ...
    // With no-cors, we can't check response status, so we assume success
    console.log('✅ Admin webhook notification sent to Zapier')
    return true
